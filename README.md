...
- [Knowledge-Graph-Grounded, Cost-Aware Decision Agent for Materials Discovery](#knowledge-graph-grounded-cost-aware-decision-agent-for-materials-discovery)
  ...
  - [Stretch Goals / Further Potential Improvements](#stretch-goals--further-potential-improvements)
...
**Important.** CGCNN's apparent win is largely a property of the benchmark, not evidence that it models the physics better. CGCNN was trained on MP's target values and absorbed whatever correction scheme is in them; MACE never saw those targets. MACE's fair headline figure is the **non-oxide MAE of 0.114 eV/atom** — the overall figure is dominated by the artefact described next.
...
> **NOT reproducible from this repo.** Run via
> [MLIP Studio](https://mlip-studio.streamlit.app/), model **UMA Small 1.2**,
> task head **`omat`** (the materials head — `omol` is for molecules and would
> be wrong for a periodic crystal), task *Cell + Geometry Optimization*,
> LBFGS, fmax 0.05 eV/Å, max 50 steps.

**Cross-model check: the force signal is not a MACE artifact.** Six materials —
the five highest-force in the corpus plus Ni₂P as a low-force reference — were
relaxed with a different foundation model and its step-0 max force compared against MACE's:

| mpid | Formula | MACE (eV/Å) | UMA (eV/Å) |
|---|---|---|---|
| mp-1281832 | CoO₂ | 0.985 | 1.088 |
| mp-644514 | MnO₂ | 0.767 | 0.859 |
| mp-943 | Co₃S₄ | 0.795 | 0.661 |
| mp-1022963 | IrO₃ | 0.736 | 0.467 |
| mp-754064 | MnP₂ | 0.681 | 0.439 |
| mp-21167 | Ni₂P | 0.034 | 0.083 |

**r = +0.878**, near-identical rank ordering, and Ni₂P an order of magnitude
below the rest in both. Two models with different architectures and training
data disagree with the MP geometries in the same places, so the gate is
measuring a property of the structures rather than of MACE.

This is *not* supporting the oxide formation-energy
finding above: that is an energy phenomenon and this is a force one, and they
merely concentrate in the same chemistry. Had UMA reported low forces where
MACE reported high, the two would have been linked, but it didn't. n = 6, drawn
from the tail rather than sampled, so this is a spot check on what the signal
means, not a validation of where the threshold sits. Full table, convergence
data and reproducibility caveats in
`notebooks/UMA_relaxation_showcase.ipynb`.
...
## Stretch Goals / Further Potential Improvements
...
- Persist per-campaign candidate ordering so a campaign trace can be replayed deterministically despite the Planner being an LLM.
